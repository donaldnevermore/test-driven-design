...
public class Process {
    public static void Start() {
        var messagesJson = File.ReadAllText(AppContext.BaseDirectory + "assets/messages.json");
        var messages = JsonSerializer.Deserialize<List<JsonElement>>(messagesJson);
        if (messages is null) {
            throw new Exception("messages cannot be null.");
        }

        var descriptionJson = File.ReadAllText(AppContext.BaseDirectory + "assets/description.json");
